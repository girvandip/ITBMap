...
class Util {
    ...
    public:
        ...
        static void mergeFrameBuffer(int ***frameBuffer1, int ***frameBuffer2, int ***framebufferResult) {
            int fbfd = 0;
            struct fb_var_screeninfo vinfo;
            struct fb_fix_screeninfo finfo;
            long int screensize = 0;
            ...
            for(int i = 0; i < vinfo.yres; i++){
                for(int j = 0; j < vinfo.xres; j++){
                    for(int k = 0; k < 4; k++){
                        if(frameBuffer1[i][j][k] > frameBuffer2[i][j][k]) {
                            framebufferResult[i][j][k] = frameBuffer1[i][j][k];
                        } else {
                            framebufferResult[i][j][k] = frameBuffer2[i][j][k];
                        }
                    }
                }
            }
